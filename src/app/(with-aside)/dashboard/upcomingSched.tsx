...
const UpcomingSchedule = ({
    ...
    const todaySchedules = schedules
        .filter(schedule => isToday(new Date(schedule.startDate as unknown as string)))
        .sort((a, b) => new Date(a.startDate as unknown as string).getTime() - new Date(b.startDate as unknown as string).getTime());

    const tomorrowSchedules = schedules
        .filter(schedule => isTomorrow(new Date(schedule.startDate as unknown as string)))
        .sort((a, b) => new Date(a.startDate as unknown as string).getTime() - new Date(b.startDate as unknown as string).getTime());
            ...
            <ScrollArea className='h-64'>
                <div className='flex flex-col text-center font-medium'>
                    ...
                    {tomorrowSchedules.length === 0 && (
                        <p className="text-xs text-muted-foreground border-y border-y-abu-1 py-3">No schedules
                            tomorrow</p>
                    )}
                </div>
            </ScrollArea>
