...
import {eachDayOfInterval, endOfMonth, format, getDay, getDaysInMonth, startOfMonth,} from 'date-fns';
...
export default function Calendar({events}: { events?: EventMap[] }) {
    ...
    return (
        <div
            className='min-w-fit flex-1 bg-white rounded-2xl w-full md:w-auto shadow-md gap-4 p-6 flex flex-col md:min-h-[50vh] min-h-[30vh]'>
            ...
            <div className='grid grid-cols-7 text-sm border border-abu-1 flex-1'>
                ...
                {days.map((day) => (
                    <Popover key={day.getTime()}>
                        ...
                        <PopoverContent
                            className='px-0 rounded-xl shadow-md text-xs py-2 border border-navy divide-y-[1px]'>
                            {events
