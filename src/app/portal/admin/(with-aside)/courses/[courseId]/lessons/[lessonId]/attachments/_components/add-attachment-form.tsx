...
export default function AddAttachmentForm({courseId, lessonId}: AddAttachmentFormProps) {
    ...
    const {execute: executeCreate, status} = useAction(createAttachment, {
        ...
        onError: ({error: {fetchError, validationErrors}}) => {
            toast.error(fetchError || validationErrorToString(validationErrors) || 'Failed to create attachment');
        },
    });
    ...
    const {
        ...
        setValue,
        watch,
    } = useForm<z.infer<typeof addAttachmentSchema>>({
        defaultValues: {
            name: '',
            description: '',
            file: undefined,
