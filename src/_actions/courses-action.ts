...
export const addCategoryCourse = actionClient
  ...
  .action(async ({ parsedInput }) => {
    try {
      ...
      let json: { data?: CategoryResponse; error?: string } = {};
      try {
        json = await res.json();
      } catch (e) {
        throw new PWAError('Failed to parse response');
      }
      if (!res.ok) {
      ...
      return (json.data ?? undefined) as Partial<CategoryResponse>;
    } catch (err) {
