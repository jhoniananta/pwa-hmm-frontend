...
export default async function AvatarUpload({currentAvatar, name}: AvatarUploadProps) {
    ...
    const {execute: executeUpload, isExecuting} = useAction(uploadProfileImage, {
        ...
        onError: ({error: {fetchError, validationErrors}}) => {
            toast.error(fetchError || validationErrorToString(validationErrors) || 'Failed to upload image');
        },
    });
    ...
    return (
        <>
            <div
                ...
                <EditProfile/>
                <div
                    className='flex items-center justify-center p-1.5 bg-white w-fit rounded-full absolute left-[7.5%] bottom-0 translate-y-1/2'>
